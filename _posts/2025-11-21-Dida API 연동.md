---
layout: post
title: 올마이투어 Dida 외부 API 연동 정리
date: 2025-11-21 00:00:00 +0900
categories: [allmytour, dida]
tags: [dida, api, hotel, booking]
---

# 올마이투어 Dida 외부 API 연동 정리

올마이투어 서비스에서 Dida(디다) 외부 API를 연동하는 `dida.allmytour` 프로젝트의 API 목록 및 처리 프로세스를 정리한다.
...
- **인증**: ClientID + LicenseKey (`Header`에 포함)
...
| **처리 요약** | 올마이투어 호텔코드 리스트로 각 호텔의 최저가 조회 |

**프로세스**

1. 요청으로 받은 올마이투어 호텔코드로 DB에서 Dida 호텔ID 목록 조회
...
1. 올마이투어 상품 ID로 DB에서 Dida 호텔정보 조회
...
2. Dida `CreateBooking` API 호출
   - 요청 파라미터: `CheckInDate`, `CheckOutDate`, `NumOfRooms`, `ClientReference`(올마이투어 예약번호), `ReferenceNo`, `Contact`(예약자 정보), `GuestList`(투숙객 정보)
...
### 2-8. 예약 조회 (SearchBooking - 올마이투어 예약번호 기준)

| 항목 | 내용 |
|------|------|
| **Endpoint** | `GET /api/realtime/search-booking/{allMyTourBookingId}` |
| **Dida API** | `channel.dida.webClient.SearchBooking` |
| **처리 요약** | 올마이투어 예약번호로 DB에서 Dida BookingID 조회 후 예약 상세 반환 |

**프로세스**

1. DB에서 올마이투어 예약번호(`allMyTourBookingId`)로 Dida `BookingID` 조회
...
| 항목 | 내용 |
|------|------|
| **Endpoint** | `GET /api/realtime/search-booking-didaID/{allMyTourBookingId}` |
| **Dida API** | `channel.dida.webClient.SearchBooking` |
...
| 8 | GET `/search-booking/{id}` | SearchBooking | 예약조회 (올마이투어 예약번호) |
...
**인증**  
모든 요청의 Body에 `Header: { ClientID, LicenseKey }` 포함 (`RequestUtils.getNotHttpEntity()` / `getHttpEntity()`).
